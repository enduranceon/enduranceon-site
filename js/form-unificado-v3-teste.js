document.addEventListener("DOMContentLoaded", function() {
    console.log("[FORM UNIFICADO V3 TESTE] Script de diagnóstico carregado.");

    const form = document.getElementById("unified-form");
    if (!form) {
        console.error("[ERRO FORM UNIFICADO V3 TESTE] Formulário #unified-form não encontrado.");
        return;
    }
    ...
    if (!nomeInput || !whatsappInput || !modalidadeSelect || !planoSelect || !periodicidadeSelect || !treinadorSelect || !termosCheckbox) {
        console.error("[ERRO FORM UNIFICADO V3 TESTE] Um ou mais elementos essenciais do formulário não foram encontrados.");
        return;
    }
    console.log("[FORM UNIFICADO V3 TESTE] Todos os elementos do formulário foram encontrados.");
    ...
    form.addEventListener("submit", function(event) {
        ...
        console.log("[FORM UNIFICADO V3 TESTE] Evento de submit disparado e prevenido.");

        // Validações básicas
        if (!nomeInput.value.trim() || !whatsappInput.value.trim() || !modalidadeSelect.value || !planoSelect.value || !periodicidadeSelect.value || !treinadorSelect.value || !termosCheckbox.checked) {
            alert("Por favor, preencha todos os campos obrigatórios e aceite os termos.");
            console.warn("[FORM UNIFICADO V3 TESTE] Validação falhou. Campos não preenchidos ou termos não aceitos.");
            return false;
        }
        console.log("[FORM UNIFICADO V3 TESTE] Validações passaram.");
        ...
        console.log("--- DADOS SELECIONADOS PARA TESTE ---");
        ...
        if (tecnoFitLinks[modalidade] && tecnoFitLinks[modalidade][plano] && tecnoFitLinks[modalidade][plano][periodicidade]) {
            ...
            console.log(`[FORM UNIFICADO V3 TESTE] URL Tecnofit QUE SERIA ABERTA: ${checkoutUrl}`);
        } else {
            console.error(`[ERRO FORM UNIFICADO V3 TESTE] URL Tecnofit NÃO seria encontrada para: Modalidade=
'${modalidade}', Plano='${plano}', Periodicidade='${periodicidade}'.`);
        }

        console.log("[FORM UNIFICADO V3 TESTE] Nenhuma ação de envio ao Formspree ou abertura de link será executada neste teste.");
        alert("Teste de diagnóstico: Nenhuma ação de envio ou redirecionamento foi executada. Verifique o console do navegador para os logs.");
        
        return false; // Reforça a prevenção do comportamento padrão.
    });

    console.log("[FORM UNIFICADO V3 TESTE] Script de diagnóstico totalmente inicializado.");
});
