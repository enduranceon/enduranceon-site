// Aplicativo de Teste rFTPa - Endurance On
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const distancia3minInput = document.getElementById('distancia-3min');
    const pace9minInput = document.getElementById('pace-9min');
    const distancia9minInput = document.getElementById('distancia-9min');
    ...
    // Adicionar eventos para calcular pace automaticamente quando a distância é inserida
    distancia3minInput.addEventListener('input', () => {
        if (distancia3minInput.value) {
            // Calcular pace para o intervalo de 3 minutos
            const distanciaKm = parseFloat(distancia3minInput.value) / 1000;
            const tempoMinutos = 3; // 3 minutos fixos
            const paceMinutosKm = tempoMinutos / distanciaKm;
            
            // Converter para formato min:seg
            const paceMin = Math.floor(paceMinutosKm);
            const paceSeg = Math.round((paceMinutosKm - paceMin) * 60);
            
            // Atualizar campo de pace
            pace3minInput.value = `${paceMin}:${paceSeg.toString().padStart(2, '0')}`;
        }
    });
    
    distancia9minInput.addEventListener('input', () => {
        if (distancia9minInput.value) {
            // Calcular pace para o intervalo de 9 minutos
            const distanciaKm = parseFloat(distancia9minInput.value) / 1000;
            const tempoMinutos = 9; // 9 minutos fixos
            const paceMinutosKm = tempoMinutos / distanciaKm;
            ...
            // Atualizar campo de pace
            pace9minInput.value = `${paceMin}:${paceSeg.toString().padStart(2, '0')}`;
        }
    });
    ...
    calcularRftpaBtn.addEventListener('click', () => {
        ...
        if (!pace3minInput.value || !pace9minInput.value) {
            alert('Por favor, insira o pace ou a distância para ambos os intervalos.');
            return;
        }
